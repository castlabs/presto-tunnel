...
class Tunnel extends http.Agent {
    ...
    _findAvailableConnection() {
        for (let i = 0; i < this.connections.length; i++) {
            if (!this.connections[i].connected()) return this.connections[i];
        }
        return null;
    }
 ...
 */
class TunnelServer {
    constructor(serverName, port, tunnelScheme, subDomains = false) {
...
function startServer(serverName, port, tunnelScheme, subDomains = false) {
    let tunnelServer = new TunnelServer(serverName, port, tunnelScheme, subDomains);
    tunnelServer.start()
}
