...
class Tunnel extends http.Agent {
    constructor(id, name, url, maxSlots) {
        super({
            keepAlive: false,
            maxFreeSockets: 1,
        });
 ...
 */
class TunnelServer {
    constructor(serverName, port, tunnelScheme, subDomains = false, graceTimeout = 2) {
        this._serverName = serverName;
        this._port = port;
        this._tunnelScheme = tunnelScheme;
        this._tunnels = [];
        this._debug = require('debug')('tl:server');
        this._tunnelNameSeparator = subDomains ? '.' : '-';
        this._maxSlots = 2; // TODO: configure me
        this._tunnelCloseTimeouts = {};
        this._tunnelGraceTimeout = (graceTimeout || 2) * 1000;
    }
    ...
    start() {
        ...
        server.listen(this._port, () => {
            this._debug(`Server started on ${this._port}`);
        });
    }
    ...
    _createHttpServer() {
        return http.createServer((req, res) => {
            ...
            if (hostName === this._serverName && parsed.pathname === '/' && newTunnelRequested) {
                ...
                this._createNewTunnel(preferredTunnelName, res);
     ...
     * @private
     */
    _createNewTunnel(preferredTunnelName, res) {
        ...
        const tunnelUrl = `${this._tunnelScheme}://${tunnelName}${this._tunnelNameSeparator}${this._serverName}`;
        this._debug('Create new tunnel connection to ' + tunnelUrl)
        const tunnel = new Tunnel(tunnelId, tunnelName, tunnelUrl, this._maxSlots);
    ...
    _handleWebSocketRequest(request) {
        ...
        if (!tunnel.addConnection(connection)) {
            this._debug(`Tunnel connection no accepted by tunnel ${tunnel.id}.`)
            connection.sendUTF('no-slots');
            connection.close();
        } else {
...
function startServer(serverName, port, tunnelScheme, subDomains = false, graceTimeout = 2) {
    let tunnelServer = new TunnelServer(serverName, port, tunnelScheme, subDomains, graceTimeout);
    tunnelServer.start()
}
