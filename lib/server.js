...
class WsStream extends stream.Duplex {
    ...
    handleWsMessage_(message) {
        if (message.type === 'utf8') {
            ...
            this.debug('Read ' + message.binaryData.length + ' bytes');
...
class TunnelAgent extends http.Agent {
    ...
    createConnection(options, cb) {
        ...
        this.debug('Creating stream. Available connections left %s', this.availableSockets.length);
...
function startServer(serverName, port, tunnelScheme, subDomains = false) {
    ...
    wsServer.on('request', function (request) {
        ...
        if(hostName !== serverName) {
            debug('Forwarding WebSocket connection not support!');
            request.reject();
            return;
        }
