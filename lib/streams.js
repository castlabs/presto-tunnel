...
const COMMANDS = {
    ...
    // Command issues when the server received a request that
...
class WebsocketStream extends stream.Duplex {
    ...
    _handleWsMessage(message) {
        if (message.type === 'utf8') {
            ...
            // we need to handle the REQUEST command separately since we
    ...
    setRequestId(id) {
        ...
        this.ws.sendUTF(createCommand(COMMANDS.REQUEST, id));
    }
