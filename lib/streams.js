...
class WebsocketStream extends stream.Duplex {
    ...
    _handleWsMessage(message) {
        if (message.type === 'utf8') {
            if (message.utf8Data.startsWith('tcp-terminated:')) {
                let targetId = message.utf8Data.split('tcp-terminated:')[1];
                if (targetId !== this.requestId) {
                    this._debug('Client Side TCP connection terminated but channel IDs do not match',
                        targetId, this.requestId, message.utf8Data)
                } else {
                    this._debug('Client Side TCP connection terminated for', targetId);
                    this.end();
                }
            } else if (message.utf8Data === 'END') {
                this._debug('Server Side requested connection termination');
                this.emit('terminate');
            } else if (message.utf8Data.startsWith("RID:")) {
                const mid = message.utf8Data.split("RID:")[1];
                this.requestId = mid;
                this._debug('Received request id', mid)
            } else {
                try {
...
class LocalStream extends stream.Duplex {
    ...
    _createLocalStream(chunk, encoding, callback) {
        ...
        this._debug(`Creating local connection ${this._host}:${this._port}`)
        ...
        connection.once('connect', () => {
            this._debug(`Connection to local port ${this._port} established.`);
            if (LOG_DATA) {
                this._debug_data('Write ' + chunk.length + ' bytes');
            }
        ...
        connection.on('close', () => {
            if (this._localConnection != null) {
                this._debug('Closing local connection after close');
        ...
        connection.on('end', () => {
            if (this._localConnection != null) {
                this._debug('Closing local connection after end');
                this._closeLocalStream();
                this.emit('end');
            }
        });
