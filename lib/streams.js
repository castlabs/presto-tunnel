    ...
    _createLocalStream(chunk, encoding, callback) {
        ...
        connection.on('error', (err) => {
            ...
            if (this._open) {
                ...
                // there is not local stream and we are not open,
                // but we need to make sure that we emit an 'end'
                // event so it goes through the chain and informs the
                // remote side that the stream is closed.
                //
                // This can happen for instance when we get a
                // connection refused and something internal (_open comes
                // from the parent) closed the stream already.
                this.emit('end');
            }
        });
    }
