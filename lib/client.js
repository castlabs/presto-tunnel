...
const {ServerWsStream} = require("./streams");
...
class LocalStream extends stream.Duplex {
    ...
    _write(chunk, encoding, callback) {
        ...
        if (this._localConnection == null) {
            // we received data but have no local connection available
            //
            // Create the connection to the local port
            let connection = net.connect(this._port, '127.0.0.1');
...
class ClientTunnel extends EventEmitter {
    constructor(id, slots, localPort, tunnelServerUrl, tunnelUrl) {
        super();
        this._id = id;
    ...
    _createConnection() {
        ...
        client.on('connectFailed', (error) => {
            ...
            this._handleWsConnectionFailed.bind(error, client);
        });

        client.on('connect', (connection) => {
            this._debug(`WebSocket for slot  ${slotIdx} connected. ${this._connectionStatusString()}`);
            client.removeAllListeners();
            let remoteStream = new ServerWsStream(connection, slotIdx);
...
async function createTunnel(tunnelHost, localPort, preferredName) {
    ...
    let tunnel = new ClientTunnel(tunnelData.tunnelId, tunnelData.maxSlots, localPort, tunnelHost, tunnelData.tunnelUrl);
    tunnel.connect();
    return tunnel;
}
